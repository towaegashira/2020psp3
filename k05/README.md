...
l.106 深さ優先探索を以下のように実行する

　l.110 visitedを初期化（値をすべて0にする）

　l.117 Stackを初期化

　l.118 出発点をPush

　l.120 Stackから取り出すものがなくなるまで以下の処理を実行

　　l.122 Stackから値を取だし、valに代入する

　　l.124 valの場所に訪れたことがない場合以下の処理を実行する

　　　l.126 行ったっことがあることにする

　　　l.127 その場所から行ける場所をすべてStackの中に入れる

　l.137 行ったことがある場所だけ表示する

l.205 幅優先探索を以下のように実行する

　l.209 visitedを初期化（値をすべて0にする）

　l.217 キューを初期化

　l.218 出発点をキューに入れる

　l.220 キューから取り出すものがなくなるまで以下の処理を実行する

　　l.222 キューから値を取り出し、valに代入

　　l.224 valの場所に行ったことがない場合、以下の処理を実行する

　　　l.226 行ったことがあることにする

　　　l.227 その場所から行ける場所をすべてキューにPushする

　l.238 行ったことがある場所をすべて表示する
