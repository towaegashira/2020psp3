...
l.83 行数がメニューの数以下の時以下の処理をループする

 l.84 商品の値段が所持金より高い場合は一つ上の行の値をコピー

  l.87 商品を入れ替えた時、入れ替えない場合の取得カロリーより高い場合、入れ替えたときのカロリーを表に入力

  l.91 入れ替えない場合の取得カロリーのほうが高い場合、その値を表に入力
